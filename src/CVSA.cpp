...
bool CVSA::configure(void){
    ...
    if(ros::param::get("~band", band_str) == false){
        ROS_ERROR("[Processing] Missing 'band' parameter, which is a mandatory parameter");
        return false;
    }
    if(!str2vecOfvec<float>(band_str, band)){
        ROS_ERROR("[Processing] Error in 'band' parameter");
        return false;
    }
    ...
    for(int i = 0; i < band.size(); i++){
        this->filters_low_.push_back(rosneuro::Butterworth<float>(rosneuro::ButterType::LowPass,  filterOrder,  band[i][1], sampleRate));
        this->filters_high_.push_back(rosneuro::Butterworth<float>(rosneuro::ButterType::HighPass,  filterOrder,  band[i][0], sampleRate));
    }
...
CVSA::ClassifyResults CVSA::classify(void){
    ...
    try{
        ...
        for(int i = 0; i < this->filters_low_.size(); i++){
            ...
            // Logarithm
            data2 = data1.array().log();

            // Extract features
            Eigen::MatrixXf features = this->decoder_->getFeatures(data2);
